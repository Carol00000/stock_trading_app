class StocksController < ApplicationController
  ...
  def create # buy
    ...
    if @intraday_data && @intraday_data['Time Series (5min)']
      ...
      redirect_to intraday_stocks_path(symbol: @symbol, commit: "Search Stock"), notice: "Successfully purchased #{@shares} shares of #{@symbol}."
    else

      redirect_to intraday_stocks_path, alert: "Unable to retrieve stock data for #{@symbol}. Please try again later."
    end
  end
  ...
  def update # sell
    ...
    redirect_to intraday_stocks_path(symbol: @symbol, commit: "Search Stock")
  end
end